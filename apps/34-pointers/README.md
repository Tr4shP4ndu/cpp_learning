...
and data structures. This lesson covers what a pointer *is*; lesson 35 focuses
...
- `*ptr` **dereferences** — follows the address to the value (lesson 35).
- `int* nothing = nullptr;` — a pointer that points at nothing. Always check for
  null before dereferencing.
- `ptr = &other;` — a pointer can be re-pointed; a reference (lesson 33) cannot.
...
pointers* (lesson 37) so you rarely handle raw owning pointers by hand.

## Build & run
```sh
make run app=34-pointers      # C++
make run-c app=34-pointers    # C
```
