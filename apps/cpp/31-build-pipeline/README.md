...
Run these from **inside this folder** (`apps/31-build-pipeline`):
```sh
...
- **Linker error** — files compiled fine, but the linker can't find a
  ...
  ("duplicate symbol"). This is the ODR (lesson 30) showing up at link time.

## Build & run
```sh
make run app=31-build-pipeline   # the Makefile runs all four stages for you
```
