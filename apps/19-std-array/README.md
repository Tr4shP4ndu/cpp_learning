...
access. In C++, prefer it over raw arrays.
...
- The size `N` is part of the type: `std::array<int,3>` and `std::array<int,4>`
  ...
  (lesson 22).
- `std::array` lives on the stack (no heap allocation) — great for small,
  fixed-size data.
...
## Try it yourself
Change `ids.at(4)` to `ids.at(99)` and run — watch it throw `std::out_of_range`
(the sanitizer/terminate will report it). Then compare with `ids[99]`.
