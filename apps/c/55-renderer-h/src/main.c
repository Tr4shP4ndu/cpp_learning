...
int main(int argc, char** argv) {
    ...
    if (strcmp(modelArg, "cube") == 0) {
        ...
        fprintf(stderr, "55-renderer-h: cannot load model: %s\n", modelArg);
        img_free(&img);
        free(zbuf);
        return 1;
    }
    ...
    if (argc > 3 && strcmp(argv[3], "-") != 0) {
        ...
        if (img_read_ppm(&tex, argv[3]) != 0) {
            fprintf(stderr, "55-renderer-h: cannot load diffuse: %s\n", argv[3]);
    ...
    if (argc > 4 && strcmp(argv[4], "-") != 0) {
        ...
        if (img_read_ppm(&nm, argv[4]) != 0) {
            fprintf(stderr, "55-renderer-h: cannot load normal map: %s\n", argv[4]);
            model_free(&model); img_free(&img); free(zbuf);
            return 1;
        }
