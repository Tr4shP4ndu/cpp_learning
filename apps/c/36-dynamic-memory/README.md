...
The C version of [`apps/cpp/36-dynamic-memory`](../../cpp/36-dynamic-memory/).
...
## Compared to C++
C++ uses `new`/`delete`, but in practice you use smart pointers (lesson 37) and
containers that free automatically via RAII — you almost never call `free`/
`delete` by hand. See the [C++ version](../../cpp/36-dynamic-memory/).

## Build & run
```sh
make run-c app=36-dynamic-memory
```
